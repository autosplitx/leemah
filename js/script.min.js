  ...
  window.addEventListener("scroll", () => {
    ...
    if (topIcon) {
      ...
      if (
        ...
        let logo;
        document
          .querySelector(".top-icon .home")
          .classList.toggle("d-home", window.scrollY > 400);
      }
    }
    if (backToTop) {
...
const prodOptCont = document.querySelector("#prod-opt-cont"),
  pocBtn = document.querySelectorAll(".open-btn");
pocBtn.forEach((btn) => {
  ...
  window.addEventListener("mouseup", (e) => {
    e.target != prodOptCont &&
      e.target.parentNode == prodOptCont &&
      (prodOptCont.style.display = "none");
  });
const smallThumbs = document.querySelectorAll(".smallThumb"),
  largeThumb = document.querySelector("#lgThumb");
...
smallThumbs.forEach((sThumb) => {
  sThumb.addEventListener("mouseover", () => {
    (largeThumb.src = sThumb.src),
      smallThumbs.forEach((tab) => {
        tab.classList.remove("activeThumb");
      }),
      sThumb.classList.add("activeThumb");
  });
}),
  initAccordion(".accordion", !0);
